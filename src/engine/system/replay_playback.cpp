...
#include "engine/param/replay_reader.h"
  ...
  namespace playback
  {
    ...
    void progress()
    {
      ...
      if(s_playbackOperations.count() > (s_playbackTimestamp + 1))
      {
        ...
        if(operation == "msg")
        {
          engine::network::metadata::message::incomingMessage(s_playbackOperations[s_playbackTimestamp]);
    ...
    void setTimestamp(int index)
    {
      ...
      while(l_position != 0)
      {
        ...
        {
          engine::network::metadata::message::incomingMessage(s_playbackOperations[l_position]);
          s_replayViewport->loadCurrentMessage();
          m_MSGFound = true;
        }
